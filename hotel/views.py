...
from .models import Comments, Hotels,Rooms,Reservation, ImageRoom
...
def edit_room(request):
    ...
    if request.method == 'POST' and request.user.account_role == "owner":
        ...
        try:
            ...
            return render(request,'staff/editroom.html',{'room':old_room})
    else:
        ...
        try:
            ...
            for image in images:
                ...
                try:
                    ...
                    messages.warning(request,"Not type img!")
                    return redirect('staffpanel')
            new_room.save()
...
def view_room_staff(request):
    ...
    # stars       = Comments.objects.filter(book__room=room)
    stars = []
    for cmt in comment:
        stars.append(cmt.star)

    print(stars)
    if len(stars) != 0:
        star = round(sum(stars)/len(stars))
    else:
        star = 5
    star2 = 5 - star

    images = ImageRoom.objects.filter(room=room)


    return HttpResponse(render(request,'staff/viewroom.html',{'room':room,'reservations':reservation, 'comments':comment, 'star':range(star), 'star2':range(star2), 'images':images}))
...
def view_room_user(request):
    ...
    return HttpResponse(render(request,'user/viewroom.html',{'room':room,'reservations':reservation, 'comments':comment, 'star':range(star), 'star2':range(star2), 'images':images}))
...
def add_new_location(request):
    if request.method == "POST" and request.user.account_role == "owner":  
        ...
        hotels = Hotels.objects.all().filter(name=name, location=location)
        if hotels:
...
def book_room_page(request):
    try:
        ...
        messages.error(request,"Invalid fields!")
        return  redirect('user/bookroom.html')
